"""
Google Colab Setup Script for Uni-GCR with Research HSTU (V5 - Smart fbgemm Detection)

This version:
1. Checks PyTorch compatibility (requires restart if needed)
2. Tries multiple fbgemm installation strategies to find one with all operations
3. Falls back to fallback implementations if needed

IMPORTANT: May require a runtime restart. Follow the prompts.
"""
...
def check_fbgemm_operations():
    """Check which fbgemm operations are available."""
    import torch
...
def try_install_fbgemm_strategy(strategy_name, install_cmd):
    """Try installing fbgemm with a specific strategy."""
    ...
    # Install with this strategy
    success = run_command(install_cmd, f"Installing with {strategy_name}...", allow_fail=True)

    if not success:
        print(f"   ❌ Installation failed")
        return False

    # Check if fbgemm can be imported
    result = subprocess.run(
    ...
    if result.returncode != 0:
        print(f"   ❌ fbgemm_gpu cannot be imported")
        return False

    print(f"   ✅ fbgemm_gpu {result.stdout.strip()} installed")

    # Check operations
    result = subprocess.run(
        "python -c 'import torch; ops = [\"asynchronous_complete_cumsum\", \"dense_to_jagged\", \"jagged_to_padded_dense\"]; missing = [op for op in ops if not hasattr(torch.ops.fbgemm, op)]; print(len(missing))'",
        shell=True, capture_output=True, text=True
    )

    if result.returncode != 0:
        print(f"   ❌ Cannot check operations")
        return False

    num_missing = int(result.stdout.strip())

    if num_missing == 0:
        print(f"   ✅ All 3 operations present!")
        return True
    else:
        print(f"   ⚠️  {num_missing}/3 operations missing")
        return False
...
def phase2_install_dependencies():
    ...
    # Step 2: Try multiple fbgemm installation strategies
    print("\n📦 Step 2: Install fbgemm_gpu (trying multiple strategies)")
    print("   Goal: Find a build with all required operations")

    strategies = [
        ("PyTorch wheel index (cu121)",
         "pip install fbgemm-gpu --index-url https://download.pytorch.org/whl/cu121 --no-cache-dir"),

        ("PyPI (latest)",
         "pip install fbgemm-gpu --no-cache-dir"),

        ("PyPI (v1.3.0 - reported working)",
         "pip install fbgemm-gpu==1.3.0 --no-cache-dir"),
    ]

    fbgemm_success = False
    for strategy_name, install_cmd in strategies:
        if try_install_fbgemm_strategy(strategy_name, install_cmd):
            print(f"\n✅ Success with {strategy_name}!")
            fbgemm_success = True
            break

    if not fbgemm_success:
        print("\n⚠️  None of the strategies provided all operations")
        print("   Will rely on fallback implementations (10-30% slower)")
        # Install the PyTorch wheel index version anyway (most compatible)
        run_command(
            "pip uninstall fbgemm-gpu fbgemm-gpu-nightly -y",
        ...
        run_command(
            "pip install fbgemm-gpu --index-url https://download.pytorch.org/whl/cu121 --no-cache-dir",
            "Installing fbgemm-gpu (for other features)..."
        )
    ...
    available, missing = check_fbgemm_operations()

    for op in available:
        print(f"   ✅ {op}")
    for op in missing:
        print(f"   ❌ {op} (will use fallback)")

    if len(missing) == 0:
        print("\n🎉 All required operations available! Full native speed!")
    else:
        print(f"\n⚠️  {len(missing)}/3 operations missing")
        print("   Fallbacks will be used (10-30% slower but functionally correct)")
...
def print_summary():
    ...
    print("\nNote:")
    print("  • test_hstu_integration.py has fallback implementations")
    print("  • If native operations are missing, fallbacks activate automatically")
    print("  • Fallbacks are 10-30% slower but produce identical results")
